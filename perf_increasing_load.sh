...
do
  ...
  ./kafka/bin/kafka-producer-perf-test.sh --producer-props bootstrap.servers=localhost:9092 --topic $TOPIC_NAME --throughput $THROUGHPUT_CAP --record-size 1000 --num-records $NUM_RECORDS > /dev/null
  sleep $INTERVAL
done
